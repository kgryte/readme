...
function validate( opts, options ) {
	...
	if ( options.hasOwnProperty( 'sections' ) ) {
		opts.sections = options.sections;
		if ( !isStringArray( opts.sections ) ) {
			return new TypeError( 'invalid option. Sections option must be a string array. Option: `' + opts.sections + '`.' );
		}
	}
	return null;
